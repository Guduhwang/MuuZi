import { createDir, error, firstUpperCase, readFile, rootDir, toCamel, writeFile } from "../utils";
...
const service = {};
let list: Eps.Entity[] = [];
let creatingEps = false;
let epsPromise: Promise<any> | null = null;
...
async function getData() {
	...
	// 构建模式下，如果本地数据存在且不为空，直接使用本地数据，避免网络请求阻塞
	const isBuild = process.env.NODE_ENV === "production";
	if (isBuild && list.length > 0) {
		return;
	}
...
function createJson() {
	...
	if (isUpdate) {
		writeFile(getEpsPath("eps.json"), content);
	}
...
async function createDescribe({ list, service }: { list: Eps.Entity[]; service: any }) {
	...
	if (content != local_content) {
		// 创建 eps 描述文件
		writeFile(getEpsPath("eps.d.ts"), content);
	}
}
...
export async function createEps() {
	...
	epsPromise = (async () => {
		try {
			...
			// 创建描述文件
			await createDescribe({ service, list });
