  ...
  <div class="cl-upload__wrap" :class="[customClass]">
    ...
    >
      ...
      >
        <!-- 触发器 -->
        <template #footer>
          <div v-if="((type == 'image' || drag) && isAdd) || preventDefault" class="cl-upload__footer">
            <el-upload
              :ref="setRefs('upload')"
              action=""
              :drag="drag"
...
const props = defineProps({
  ...
  // 云端上传路径前缀
  prefixPath: String,
  // 是否阻止组件的默认行为，用于只使用组件上传逻辑，不显示状态、上传结果的情况
  preventDefault: {
    type: Boolean,
    default: false,
  },
...
// 上传前
async function onBeforeUpload(file: UploadRawFile, item?: Upload.Item) {
  function next() {
    const d: Upload.IPreloadItem = {
      uid: file.uid + '',
      size: file.size,
      name: file.name,
      file: file,
...
async function httpRequest(req: any, item?: Upload.Item) {
  ...
  const newFile = await compressFile(req.file);
  // 上传请求
  toUpload(newFile, {
