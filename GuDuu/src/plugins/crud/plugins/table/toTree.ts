 ...
 * @returns
 */
export function toTree(options: { lazy?: boolean; onRefresh?: (params: any) => Promise<any[]> } = {}) {
  return ({ exposed }) => {
    const Crud = useCrud();
      ...
      Crud.value.config.onRefresh = async (
        ...
        const req: Promise<any[]> = new Promise((resolve) => {
          if (onRefresh) {
            ...
            resolve(
              options.onRefresh ? options.onRefresh(params) : Crud.value?.service[Crud.value.dict.api.list](params),
            );
          }
        });
    ...
    const onData = (list: Item[], sort: 'desc' | 'asc') => {
      ...
      const deep = (arr: Item[]) => {
        arr.forEach((e) => {
          const nodes: { [key: number]: Item[] } = exposed.Table.value?.store.states.lazyTreeNodeMap.value || {};
