...
export default defineComponent({
  ...
  setup(props) {
    ...
    const getLabel = (item: Menu.Item) => t(item.meta?.localeKey || item.meta?.label || '');
    ...
    function renderMenu() {
      function deep(list: Menu.Item[], show?: boolean) {
        ...
        function filterMenu(item: Menu.Item): boolean {
          ...
          if (getLabel(item).toLowerCase().includes(keyWord)) return true;
        ...
        return list.filter(filterMenu).map((e) => {
          const displayLabel = getLabel(e);

          if (displayLabel.toLowerCase().includes(keyWord)) {
            show = true;
          }
