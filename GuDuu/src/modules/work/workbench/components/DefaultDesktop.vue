<template>
  <div class="default-desktop">
    ...
    <div class="desktop-left">
      <!-- 消息区域 -->
      <div ref="desktopLeft1" class="desktop-message">
        <div class="message-header">
          <div class="active-line"></div>
          <div class="message-title">Message</div>
        </div>
        <div v-html="siteStore.info?.desktopAnnouncement" class="message-content"></div>

        <div class="message-bottom">
          <div class="message-bg">
            <div class="">
              <div class="text-[12px] font-[600]">Today</div>
              <div class="text-[18px] font-[700]">{{ siteStore.info?.siteName }}</div>
            </div>
            <div class="tip-style">
              <div class="">saved me {{ messageTip.mySavedTime }} minutes</div>
              <div class="">saved my users {{ messageTip.myUsersSavedTime }} minutes too!</div>
            </div>
          </div>
          <div class="nail"></div>
        </div>
      ...
      <!-- 轮播图区域 -->
      <div ref="desktopLeft2" class="desktop-content">
        <el-carousel :interval="4000" arrow="hover" height="100%" class="full-height-carousel">
          <el-carousel-item v-for="item in carouselItems" :key="item.id">
            <div class="carousel-item" :style="{ backgroundImage: `url(${item.image})` }"></div>
          </el-carousel-item>
        </el-carousel>
      </div>
    </div>
    ...
    >
      <div class="video-player-container">
        ...
        <!-- 网页iframe -->
        <div v-if="currentVideo?.type === 0 && currentVideo.url" class="web-player">
          <iframe
            ref="webPlayer"
        ...
        <div v-if="currentVideo?.type === 0 && !currentVideo.url" class="web-player-content">
          <div v-html="currentVideo.content"></div>
        </div>
      </div>
...
const carouselItems = ref<Eps.AdsEntity[]>([]);
...
const getCarouselList = async () => {
  const res = await service.base.ads.list({
    page: 1,
    size: 10,
  });
  carouselItems.value = res;
};
// 视频数据
const videoItems = ref<VideoItem[]>([]);

// 获取左侧区域高度
const desktopLeft1 = ref<HTMLElement | null>(null);
const desktopLeft2 = ref<HTMLElement | null>(null);
const desktopRight = ref<HTMLElement | null>(null);
