...
function Render() {
  return (
    <div
      class="msgFile-wrap"
      onClick={() => downloadFileFromUrl(props.file.url, props.file.name)}
      title={props.file.name}
    >
      <div class="msgFile-icon-wrap">
        <FileIcon type={fileType.value} />
      </div>
      <div class="msgFile-content">
        <div class="msgFile-name">{props.file.name}</div>
        <div class="msgFile-size">{props.file.size}</div>
      </div>
    </div>
  );
}
