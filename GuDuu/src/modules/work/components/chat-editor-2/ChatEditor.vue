<template>
  <div class="chatEditor-wrap-2" ref="chatEditorWrapRef">
    <EditorContent :editor="editor" class="chat-textarea-2" :class="{ 'live-mode': app.isShowLive }" />
    <div class="chat-textarea-send-icon-wrap-2" :class="{ 'live-mode': app.isShowLive }" @click="handleSendMessage">
      <img alt="" :src="iconSend" class="chat-textarea-send-icon-2" />
    </div>
  </div>
</template>
...
const emit = defineEmits<{
  ...
  (e: 'updateCurrentMember', info: any): void;
...
const curMemberList = computed(() => {
  console.log('ChatEditor curAllMemberList.value:', curAllMemberList.value);
  const filtered = curAllMemberList.value.filter((member) => member.userId !== userStore.info!.id);
  console.log('ChatEditor curMemberList (filtered):', filtered);
  return filtered;
});
    ...
    return {
      Backspace: ({ editor }) => {
        ...
        if (nodeBefore && nodeBefore.type.name === 'mention') {
          // 获取 mention 节点的位置
          const mentionPos = $from.pos - nodeBefore.nodeSize;

          // 检查 mention 节点前面是否有 @ 符号
          const textBefore = doc.textBetween(Math.max(0, mentionPos - 1), mentionPos);
          if (textBefore === '@') {
            // 删除 @ 符号和 mention 节点
            editor
              .chain()
              .deleteRange({ from: mentionPos - 1, to: $from.pos })
              .run();
            return true;
          } else {
            // 只删除 mention 节点
            editor.chain().deleteRange({ from: mentionPos, to: $from.pos }).run();
            return true;
          }
        }
      ...
      Delete: ({ editor }) => {
        ...
        if (nodeAfter && nodeAfter.type.name === 'mention') {
          ...
          if (textBefore === '@') {
            ...
            editor
              .chain()
              .deleteRange({ from: mentionPos - 1, to: mentionPos + nodeAfter.nodeSize })
              .run();
            return true;
          } else {
            // 只删除 mention 节点
            editor
              .chain()
              .deleteRange({ from: mentionPos, to: mentionPos + nodeAfter.nodeSize })
              .run();
            return true;
          }
        }
...
//       console.log('插入内容', missingContent);
//       console.log('插入的时候moreInsPos', moreInsPos);
...
//     iconSend.value = '/svg/send.svg';
//   }
// }

function handleSendMessage() {
  console.log('点击发送时001', curMentionMember.value);
  if (props.showInputList.length > 0) {
    console.log('走新的发送');
    emit('onSubmit', editor.value!.getJSON());
  } else {
    handleSendMessageOld();
  }
}

function handleSendMessageOld() {
  // 当任何弹窗都没有的时候，emit('enter');
  if (
    (!chatInputFormComponentPopup?.length || chatInputFormComponentPopup[0]?.state?.isDestroyed) &&
    isMentionPopInactive()
...
function cleanEditor() {
  editor.value?.commands.setContent('');
  iconSend.value = '/svg/send.svg';
  // 清除当前 mention 成员，确保下次 @ 时可以正常显示选择框
  curMentionMember.value = null;
  // 确保编辑器获得焦点，以便下次输入 @ 时可以正常触发 mention 扩展
  nextTick(() => {
...
const iconSend = ref<string>('/svg/send.svg');
const editor = useEditor({
  ...
  autofocus: true,
  onUpdate: ({ editor }) => {
    console.log('更新--onUpdate', editor);
    const isEmpty = editor.isEmpty;
    if (isEmpty) {
      iconSend.value = '/svg/send.svg';
    } else {
      iconSend.value = '/svg/send-hover.svg';
    }
    // HTML
  ...
  editorProps: {
    ...
    handleDOMEvents: {
      ...
      compositionend: (view, event) => {
        // 使用 setTimeout 确保 compositionend 事件处理完成后再检查
        setTimeout(() => {
          ...
          // 如果光标前是 @ 字符，手动删除并重新插入以触发 mention 建议
          if (textBefore === '@') {
            // 删除光标前的 @
            editor.value
              .chain()
              .deleteRange({ from: $from.pos - 1, to: $from.pos })
              .insertContent('@') // 重新插入 @，这会触发 TipTap 的输入处理
              .run();
          }
        }, 0);
...
function showChatInputForm(
  ...
  console.log('当前成员的数据', curMentionMember.value);
  console.log('展示表单组件', params);
  // 发布推特类型，自动设置一个图片参数
...
function updateCurrentMember(value) {
  emit('updateCurrentMember', value);
}
