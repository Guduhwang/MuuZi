<template>
  <div id="project" :key="key" class="lg:mx-5">
    <span class="flex text-sm my-3">
      <h3 v-if="index == null" class="text-purplefy font-fira_bold mr-3">{{ project.name }}</h3>
      <h3 v-else class="text-purplefy font-fira_bold mr-3">Project</h3>
    ...
    <div id="project-card" class="flex flex-col">
      <div id="window">
        <div class="absolute flex right-3 top-3">
          <img :src="project.logo" alt="" class="w-6 h-6 mx-1 hover:opacity-75" />
        </div>
        <img id="showcase" :src="project.image" alt="" class="" />
      </div>
