<template>
  <div id="console">
    ...
    <div id="console-menu" class="h-full flex flex-col items-end justify-between">
      ...
      <!-- skip -->
      <a id="skip-btn" @click="skip" class="font-fira_retina flex hover:bg-white/20"> skip </a>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    render() {
      const gameScreen = this.$refs.gameScreen;
      ...
      for (let i = 0; i < 40; i++) {
        ...
        for (let j = 0; j < 24; j++) {
          ...
          const snakeCell = this.snake.find((snakeCell) => snakeCell.x === j && snakeCell.y === i);
