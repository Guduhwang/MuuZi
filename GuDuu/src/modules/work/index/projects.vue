  ...
  <main class="flex flex-col flex-auto lg:flex-row overflow-hidden">
      ...
      <nav id="filters" class="w-full flex-col">
        <div v-for="tech in techs" :key="tech" class="flex items-center py-2">
          ...
          <img
            ...
            alt=""
            class="tech-icon w-5 h-5 mx-4 hidden"
          />
          <span :id="'title-tech-' + tech" style="margin-left: 10px">{{ tech }}</span>
        </div>
      </nav>
    </div>
    ...
    <div class="flex flex-col w-full overflow-hidden">
      ...
      <div id="projects-case" class="grid grid-cols-1 lg:grid-cols-2 max-w-full h-full overflow-scroll lg:self-center">
        ...
        <ProjectCard v-for="(project, key, index) in projects" :key="key" :index="index" :project="project" />
      </div>
    </div>
  </main>
</template>
...
const techs = ['Investment Projects', 'Internal Incubation Projects', 'Collaboration Projects'];
...
const getList = async () => {
  const res = await axios.post('/api/index-api/admin/demo/project/page', {
    page: 1,
    size: 100,
    sort: 'desc',
    order: 'createTime',
  ...
  projects2.value = res.data.data.list;
};
...
function filterProjects(tech) {
  ...
  filters.value[0] == 'all' ? (projects.value = projects2.value) : (projects.value = filterProjectsBy(filters.value));
...
function filterProjectsBy(filters) {
  const projectArray = Object.values(projects2.value);
  return projectArray.filter((project) => {
    return filters.some((filter) => project.category.includes(filter));
  });
}
