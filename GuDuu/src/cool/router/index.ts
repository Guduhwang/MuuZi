...
const routes: RouteRecordRaw[] = [
  ...
  //   path: '/',
  //   name: 'index',
  //   component: () => import('/src/modules/work/workbench/index.vue'),
  //   redirect: '/workbench',
  //   children: [],
  // },
  {
...
router.find = function (path: string) {
  ...
  matcher.getRoutes().find((route) => {
    ...
    if (routeRegex.test(path)) {
      if (path === '/admin') {
        // 如果路径是 /admin，优先查找 /admin 路由本身
        matchedRoute = routeList.find((r) => r.path === '/admin' || r.name === 'admin');
        // 如果找不到 /admin 路由，再查找标记为首页的路由
        if (!matchedRoute) {
...
router.beforeEach(async (to, from, next) => {
  ...
  // 获取用户和进程数据
  const { userStore, process } = useBase();
  ...
  // 如果路由不存在
  if (!route) {
    next(userStore.token ? '/404' : '/login'); // 根据用户登录状态重定向
    return;
  }
  ...
  if (userStore.token) {
    ...
    if (!storage.isExpired('token')) {
      if (userStore.info) {
        // 暂时用industryId来判断是否补充过信息  改成nickName判断是否补充过信息
        if (!userStore.info.nickName) {
          let url =
            '/register?isShowAdditionalInfo=' + (to.query.isShowAdditionalInfo || '1') + '&id=' + userStore.info.id;
          // 暂时用googleId
          if (!userStore.info?.inviteCode) {
            url += '&isNeedInviteCode=1';
          }
          if (url !== to.fullPath) {
            next(url);
          } else {
        ...
        // 【权限检查】如果访问 /admin 路径，检查用户类型是否包含 1
        if (to.path.startsWith('/admin')) {
          // 如果用户类型不是 1
          if (
            !userStore.info.roleIds ||
            !Array.isArray(userStore.info.roleIds) ||
            !userStore.info.roleIds.includes(1)
          ) {
            // 跳转到404
            next('/404');
            return;
    ...
    const path = to.path;
    if (!config.ignore.token.some((ignorePath) => path.includes(ignorePath))) {
      next('/login');
      return;
    }
  }
